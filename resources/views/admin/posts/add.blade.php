...
@section('title', $edit ? 'تعديل' : 'إضافة جديد')
...
<form method="POST">
    <div class="row">
        <div class="card bg-light-subtle shadow-none position-relative overflow-hidden mb-4">
            <div class="card-body px-4 py-3">
                <div class="row align-items-center">
                    <div class="col-12">
                        <h4 class="fw-semibold mb-8">{{$edit ? 'تعديل' : 'إضافة جديد'}}</h4>
        ...
        <div class="col-xl-9">
            <div class="card border">
                <div class="card-body p-12">
                    ...
                    @endif
                    <input type="hidden" name="meta[primary_category]" id="primary_category" value="{{ $edit && $post->PrimaryCategory() ? $post->PrimaryCategory()->ID : '' }}">
                    <div class="mb-0">
                        <div class="row gx-10 mb-3" style="direction:ltr">
                            <div class="col-lg-12">
                                ...
                                <div class="mb-3">
                                    <input type="text" class="form-control form-control-solid" placeholder="العنوان" name="post[post_title]" value="{{ $edit ? $post->post_title : '' }}"/>
                                </div>
                                <label class="form-label fs-4 fw-bolder text-gray-700 mb-3">
                                    الرابط
                                </label>
                                <div class="mb-3">
                                    <input type="text" class="form-control form-control-solid" placeholder="الرابط" name="post[post_slug]" value="{{ $edit ? $post->post_slug : '' }}"/>
                                </div>
                                <label class="form-label fs-4 fw-bolder text-gray-700 mb-3">
                                    الوصف
                                </label>
                                <div class="mb-3">
                                    <textarea class="tinymce" id="editor" name="post[post_content]">{!! $edit ? $post->post_content : '' !!}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    <div class="tab-content" id="myTabContent" style="direction: ltr;text-align:left">
                        ...
                        <div class="tab-pane fade" id="data-tab" role="tabpanel">
                            @include('admin.posts.meta.data-product', ['data' => $edit ? \App\Models\PostMeta::getMeta($post->ID,'data') : null])
                        </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="col-xl-3">
            ...
            <div class="card border">
                <div class="card-body p-5">
                    ...
                    <div class="image-input image-input-outline w-100 text-center">
                        <img class="image-input-wrapper w-125px h-125px my-2" style="width:100px" id="featured-image" src="{{ $edit && !empty($post->post_image) ? asset('assets/uploads/'.$post->post_image) : \App\Models\Media::default() }}" style="object-fit: cover;">
                    ...
                    <div class="mb-0">
                        ...
                        <div class="row mb-3">
                            <div class="col">
                                <a href="{{ $post->url() }}" class="btn btn-dark text-light w-100" target="_blank">
                                    عرض
            ...
            @foreach(config('sidebar') as $sidebarItem)
            ...
            @endforeach
                    ...
                    <div class="card-body p-5">
                        ...
                        <div class="scroll-y mh-200px mh-lg-325px">
                            <select class="form-select form-select-solid" data-control="select2" name="meta[template]" onchange="$(this).val() == 'filter-page' ? $(this).val() == $('#filter-template-sub-items').show() : $(this).val() == $('#filter-template-sub-items').hide();">
                                ...
                                @foreach(array_diff(scandir(resource_path('views/templates')), array('.', '..')) as $temp)
                                    <option {{ $edit && $post->meta('template') == explode('.', $temp)[0] ? 'selected' : '' }} value="{{ explode('.', $temp)[0] }}">{{ explode('.', $temp)[0] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div id="filter-template-sub-items" style="display: {{ isset($post) && $post->meta('template') == 'filter-page' ? 'block' : 'none' }};">
                            <div class="separator separator-dashed my-4"></div>
                            ...
                            <select class="form-select form-select-solid" data-control="select2" name="meta[filter_category][]" multiple>
                                @foreach(\App\Models\Term::where('taxonomy', 'product_category')->get() as $term)
                                    <option {{ $edit && in_array($term->ID, is_array(json_decode($post->meta('filter_category'), 1)) ? json_decode($post->meta('filter_category'), 1) : [])  ? 'selected' : '' }} value="{{ $term->ID }}">{{ $term->name }}</option>
                                @endforeach
                            </select>
                            <div class="separator separator-dashed my-4"></div>
                            ...
                            <select class="form-select form-select-solid" data-control="select2" name="meta[filter_pages][]" multiple>
                                @foreach(\App\Models\Post::where('post_type', 'page')->get() as $page)
                                    <option {{ $edit && is_array(json_decode($post->meta('filter_pages'), 1)) && in_array($page->ID, json_decode($post->meta('filter_pages'), 1)) ? 'selected' : '' }} value="{{ $page->ID }}">{{ $page->post_title }}</option>
                                @endforeach
                            </select>
                            <div class="separator separator-dashed my-4"></div>
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    <input type="number" class="form-select-solid" name="meta[duration_limit_min]" value="{{ $edit && $post->meta('duration_limit_min') ? $post->meta('duration_limit_min') : '' }}" placeholder="Min"/>
                                </div>
                                <div class="col-lg-6">
                                    <input type="number" class="form-select-solid" name="meta[duration_limit_max]" value="{{ $edit && $post->meta('duration_limit_max') ? $post->meta('duration_limit_max') : '' }}" placeholder="Max"/>
                                </div>
                            </div>
                            <select class="form-select form-select-solid" data-control="select2" name="meta[filter_pages][]" multiple>
                                @foreach(\App\Models\Post::where('post_type', 'page')->get() as $page)
                ...
                {{-- <div class="card my-5">
                    <div class="card-body p-5">
                        <span class="card-label fw-bolder fs-3 mb-1">Booking Form</span>
                        <div class="separator separator-dashed my-4"></div>
                        <div class="scroll-y mh-200px mh-lg-325px">
                            <select class="form-select form-select-solid" data-control="select2" name="meta[booking_form]">
                                <option value="">No Template</option>
                                @foreach(\App\Models\Form::all() as $form)
                                    <option {{ $edit && $post->meta('booking_form') == $form->ID ? 'selected' : '' }} value="{{ $form->ID }}">{{ $form->title }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="card-body p-5">
                        ...
                        <div class="scroll-y mh-200px mh-lg-325px">
                            <select class="form-select form-select-solid" data-control="select2" name="post[post_parent]">
                                ...
                                @foreach(\App\Models\Post::where('post_type', 'page')->get() as $p)
                                    <option {{ $edit && $post->post_parent == $p->ID ? 'selected' : '' }} value="{{ $p->ID }}">{{ $p->post_title }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
<script>
    ...
    $('.form-check[data-taxonomy="{{ $post_type }}_category"]').each(function (){
        var catValue = $(this).find('input').val();
        if(catValue == {{ $edit && $post->PrimaryCategory() ? $post->PrimaryCategory()->ID : 0 }}){
