    ...
    <div class="row">
            ...
            <div class="card border mb-5">
                <div class="card-header border-0">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bolder fs-3 mb-1">{{ $edit ? 'Edit' : 'Add New' }} {{ $taxonomy }}</span>
                    </h3>
                </div>
                <div class="card-body py-3" style="direction:ltr">
                    
                    <div class="mb-3">
                        <input type="text" class="form-control form-control-solid" placeholder="Name" name="term[name]" value="{{ $edit ? $term->name : '' }}"/>
                    </div>
                    
                    <div class="mb-3">
                        <input type="text" class="form-control form-control-solid" placeholder="Slug" name="term[slug]" value="{{ $edit ? $term->slug : '' }}"/>
                    </div>
                    
                    <div class="mb-3">
                        <select class="form-control form-control-solid" name="term[parent]" data-control="select2">
                            <option value="0">No Parent</option>
                            @include('admin.terms.items.select',['terms' => \App\Models\Term::where('taxonomy', $taxonomy)->where('parent', 0)->get(), 'depth' => 0, 'selected' => $edit ? $term->parent : 0])
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        <select class="form-control form-control-solid" name="term[post_ID]" data-control="select2">
                            ...
                            @foreach(\App\Models\Post::get() as $post)
                                <option {{ $edit && $term->post_ID == $post->ID ? 'selected' : '' }} value="{{ $post->ID }}">{{ $post->post_title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="separator separator-dashed mb-8"></div>
                    <div class="image-input image-input-outline w-100 text-center mb-5">
                        <img class="image-input-wrapper w-125px h-125px" id="featured-image" src="{{ $edit && !empty($term->image) ? asset('assets/imgs/'.$term->image) : \App\Models\Media::default() }}" style="object-fit: cover;">
                        ...
                        <label class="btn btn-icon btn-circle btn-primary w-100 h-25px shadow" data-bs-toggle="tooltip" title="Change avatar" id="change-featured-image">
                            ...
                            <input type="hidden" name="term[image]" value="{{ $edit ? $term->image : '' }}" id="featured-image-value"/>
                        </label>
                    </div>
        ...
        <div class="col-xl-6">
            <div class="card border mb-5">
                <div class="card-header border-0">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bolder fs-3 mb-1">All {{ $taxonomy }}</span>
                    </h3>
                </div>
                ...
                <div class="card-body py-3">
                    <div class="table-responsive">
                        <table class="table align-middle gs-0 gy-4" style="direction: ltr;text-align:left">
                            ...
                            <tbody>
                                @foreach(\App\Models\Term::where('taxonomy', $taxonomy)->where('parent', 0)->get() as $term)
                                    @include('admin.terms.items.table', ['term' => $term, 'depth' => 0])
                                @endforeach
                            </tbody>
                        </table>
                    </div>
